...
export class AdminWriterChatComponent {
  ...
  constructor(private adminWriterChatService: AdminWriterChatService,
  ...
  sendMessage() {
    ...
    this.adminWriterChatService.postMessage(this.blogId, this.messageForm.value.message, this.author).subscribe((response: any) => {
      if (response.success == true) {
        this.chatMessages.push({ blogId: this.blogId, content: this.messageForm.value.message, type: 'admin-to-writer' })
